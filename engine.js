...
function weightTaxPerShaken(weightKg, isKei, ecoFlag) {
  let base;
  if (isKei) base = 6600;
  else base = Math.ceil(weightKg / 500) * 4100 * 2; // 4,100円/0.5t/年 × 2年
...
function calcTCO(car, opts, settings = DEFAULT_SETTINGS) {
  ...
  const wtPerShaken = weightTaxPerShaken(car.weight, isKei, !!car.eco);
  const wtInitial = used ? wtPerShaken : Math.round(wtPerShaken * 1.5); // 新車初回は3年分
  const legalTotal = wtInitial + initialJibai + shakens.length * (wtPerShaken + perShakenJibai);
